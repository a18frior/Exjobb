...
import GrammarDeterminer from "./data/GrammarDeterminer";
import GrammarVerbs from "./data/GrammarVerbs";

@customElement("free-lib")
export class Lib extends LitElement {
  constructor() {
    super();
    this.getRandomInt;
    this.generate_sentence;
  }

  conjunction = new Array("and", "nor", "but", "or", "yet", "both", "equally");
  modal = new Array(
    "ed",
    "s",
    "does",
  ...
  randomword(list: string[], distribution: any) {
    if (distribution) {
      var r = this.getRandomInt(0, list.length + 1);
      return list[r];
    } else {
      let ret = this.getRandomInt(0, list.length + 1);
      ret = this.getRandomInt(0, ret);
      return list[ret];
    }
  }
   ...
   */
  replacen(search: string, replace: string, subject: string, n: number) {
    var foundpos = 0;
    var beforestr = "";
    var afterstr = "";
    ...
    if (foundpos != -1) {
      ...
      subject = beforestr + replace + afterstr + n;
    }

    return subject;
  }

  countfinds(search: string, subject: string | any[]) {
    var n = 0;
    var count = 0;
    var index = 0;
    while (n >= 0) {
   ...
   *
   */
  replacerandom(placeholder: any, set: any, distribution: any, sentence: any) {
    var found = this.countfinds(placeholder, sentence);
    for (var i = 0; i < found; i++) {
      sentence = this.replacen(
        placeholder,
        this.randomword(set, distribution),
        sentence,
        0
      );
    }
   ...
   */
  public generate_sentence(
    probnounphrase: number,
    probverbphrase: number,
    probdualajdectives: number,
    distributionnouns: any,
    distributionverbs: any,
    distributionadjectives: any,
    distributionadverbs: any,
    distributiondeterminers: any,
    distributionconjunctions: any,
    distributionmodals: any
  ) {
    ...
    var nonounphrases = this.countfinds("[nounphrase]", sentence);
    for (var i = 0; i < nonounphrases; i++) {
      if (this.getRandomInt(0, 100) / 100.0 >= probnounphrase) {
        var replacestring = this.randomword(
			GrammarDeterminer,
          distributiondeterminers
        );
        ...
        sentence = this.replacen("[nounphrase]", replacestring, sentence, 0);
      } else {
        replacestring = this.randomword(
          GrammarDeterminer,
          distributiondeterminers
        );
    ...
    found = this.countfinds("[determiner] [noun]", sentence);
    ...
    nonounphrases = this.countfinds("[verbphrase]", sentence);
    for (i = 0; i < nonounphrases; i++) {
      if (this.getRandomInt(0, 100) / 100.0 >= probverbphrase) {
        sentence = this.replacen("[verbphrase]", "[verb]", sentence, 0);
      } else {
        sentence = this.replacen(
          "[verbphrase]",
          "[adverb] [verb]",
    ...
    const adjectives = this.countfinds("[adjective]", sentence);
    for (var i = 0; i < adjectives; i++) {
      if (this.getRandomInt(0, 100) / 100.0 >= probdualajdectives) {
        sentence = this.replacen(
          "[adjective]",
          "[dual adjective]",
          sentence,
    ...
    const dualadjectives = this.countfinds("[dual adjective]", sentence);
    ...
    var conjunctions = this.countfinds(
      "[adjective] [conjunction] [adjective]",
      sentence
    );
    ...
    var found = this.countfinds("[verb]", sentence);
    for (var i = 0; i < found; i++) {
      replacestring = this.randomword(GrammarVerbs, distributionverbs);
      const amodal = this.randomword(this.modal, distributionmodals);

      if (amodal == "s") {
        replacestring += "s";
      } else if (amodal == "ed") {
        replacestring += this.edify(replacestring);
      } else if (amodal == "had") {
        replacestring =
          amodal + " " + replacestring + this.edify(replacestring);
      } else {
        // All others such as can shall will did etc
        replacestring = amodal + " " + replacestring;
      }

      sentence = this.replacen("[verb]", replacestring, sentence, 0);
    }
